...
export default function AdminDashboardPage() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: metrics?.ai?.total_class || 26,
      subtext: '26 Label Master SKU AI',
      icon: FolderKanban,
      color: 'from-emerald-500 to-teal-600',
    },
    {
      title: 'Foto Dataset AI',
      value: (metrics?.ai?.total_foto_dataset || 4800).toLocaleString('id-ID'),
      subtext: 'HuggingFace Hub (Anoderb)',
      icon: Database,
      color: 'from-purple-500 to-pink-600',
    },
    {
      title: 'Versi Model AI Aktif',
      value: metrics?.ai?.model_aktif?.versi || 'v1.0',
      subtext: `Akurasi: ${((metrics?.ai?.model_aktif?.akurasi || 0.958) * 100).toFixed(1)}%`,
      icon: Cpu,
          ...
          <div className="lg:col-span-2 p-6 rounded-3xl bg-slate-900/80 border border-slate-800 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-4">
              <div>
                ...
                <p className="text-xs text-slate-400">Ringkasan MLOps visual recognition Tokiva POS</p>
              </div>
