...
export default function AdminDataCollectorPage() {
  ...
  const handleTriggerSync = async () => {
    ...
    try {
      ...
      alert(res?.pesan || 'Batch sync ke HuggingFace berhasil!');
      fetchSyncStatus();
      fetchClasses();
    } catch (err) {
  ...
  const handleSaveSyncConfig = async (e) => {
    ...
    try {
      ...
      alert('Pengaturan auto-sync berhasil disimpan!');
      setIsSyncConfigModal(false);
      fetchSyncStatus();
    } catch (err) {
      alert('Gagal menyimpan config: ' + err.message);
    }
  };
  ...
  const handleAddClass = async (e) => {
    ...
    try {
      ...
      alert('Gagal menambah class: ' + err.message);
    }
  };
  ...
  const handleExecuteMapping = async (e) => {
    ...
    try {
      ...
      if (mapMode === 'existing') {
        if (!targetClassId) return alert('Pilih class target');
        ...
        if (!newClassName) return alert('Nama class baru wajib diisi');
      ...
      alert(`Produk ${selectedUnmappedProduct.nama} berhasil dipetakan ke Class!`);
      setIsMapModal(false);
      fetchUnmapped();
      fetchClasses();
    } catch (err) {
      alert('Gagal memetakan produk: ' + err.message);
    }
  };
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full bg-slate-800 text-slate-300 text-[10px] font-mono font-semibold border border-slate-700">
                  Repo: {syncStatus?.stats?.hf_repo || 'Anoderb/dataset-collect'}
                </span>
              </div>
