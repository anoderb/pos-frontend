...
export default function AdminModelPage() {
  ...
  const webcam = useWebcam(handleFrame);
  ...
  const handleDeleteModel = async () => {
    ...
    try {
      ...
      alert(`Model ${deleteModelTarget.versi} berhasil dihapus dari database!`);
    } catch (err) {
      alert(err.response?.data?.pesan || `Gagal menghapus model: ${err.message}`);
    }
  ...
  const handleRegisterModel = async (e) => {
    e.preventDefault();
    if (!versi || !namaModel) return alert('Versi dan Nama Model wajib diisi');
    try {
      const token = getToken();
      if ((uploadMethod === 'direct_file' && selectedFiles.length > 0) || (uploadMethod === 'zip' && selectedZipFile)) {
        ...
        alert(`Model ${versi} berhasil diunggah & disinkronkan! 🎉`);
      } else {
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-200">
                {isLoading ? (
                  <tr><td colSpan={5} className="px-6 py-12 text-center text-slate-500">Memuat daftar model AI...</td></tr>
                ) : models.length === 0 ? (
